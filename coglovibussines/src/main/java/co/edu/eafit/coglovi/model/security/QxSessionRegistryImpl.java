 ...
 * HttpSessionEventPublisher} in the <tt>web.xml</tt> file so that this class is notified of
 * sessions that expire.
 * 
 * @author Ben Alex
 * @author Luke Taylor
 */
public class QxSessionRegistryImpl implements SessionRegistry, ApplicationListener<SessionDestroyedEvent> {

	// ~ Instance fields
	// ================================================================================================

	protected final Log logger = LogFactory.getLog(QxSessionRegistryImpl.class);
	...
	public List<SessionInformation> getAllSessions(Object principal, boolean includeExpiredSessions) {
		final Set<String> sessionsUsedByPrincipal = principals.get("" + ((UserDetails) principal).getUsername());
	...
	public synchronized void registerNewSession(String sessionId, Object principal) {
		...
		Set<String> sessionsUsedByPrincipal = principals.get("" + ((UserDetails) principal).getUsername());

		if (sessionsUsedByPrincipal == null) {
			sessionsUsedByPrincipal = Collections.synchronizedSet(new HashSet<String>(4));
			principals.put("" + ((UserDetails) principal).getUsername(), sessionsUsedByPrincipal);
		}
